...
// les cas, un e-mail est aussi envoyé directement via Brevo.
export async function POST(request, { params }) {
  ...
  const link = `${process.env.NEXT_PUBLIC_SITE_URL}/tenues/${meeting.id}`;
  ...
  if (friendProfile) {
    await prisma.notification.create({
      data: {
        ...
        text: `${profile.name} vous suggère une tenue à ${meeting.lodge.name} le ${meeting.date.toISOString().slice(0, 10)}.`,
      },
    });
  }
  ...
  const html = `
    <p>Bonjour,</p>
    <p><strong>${profile.name}</strong> pense que cette tenue pourrait vous intéresser :</p>
    ...
    <p><a href="${link}" style="background:#111; color:#fff; padding: 12px 22px; text-decoration:none; border-radius:6px; font-size: 13px;">Voir la tenue</a></p>
  `;
