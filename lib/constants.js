...
// Utilisée uniquement à l'AFFICHAGE (sur "Ma loge" et "Loges"), sans
// toucher aux données stockées pour les officiers — contrairement aux
// membres, dont le nom est tronqué dès la création.
export function truncateName(fullName) {
  if (!fullName) return '';
  const parts = fullName.trim().split(/\s+/);
  if (parts.length === 1) return parts[0].slice(0, 3);
