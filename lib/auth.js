...
// Les champs Decimal de Prisma (ex. Meeting.agapesPrice) ne sont pas
// des nombres JavaScript ordinaires — sans cette conversion, ils
// arrivent côté client sous forme de texte, ce qui peut casser des
// comparaisons ou calculs qui s'attendent à un nombre.
function decimalToNumber(key, value) {
  if (value && typeof value === 'object' && typeof value.toNumber === 'function' && typeof value.toFixed === 'function') {
    return value.toNumber();
  }
  return value;
}
export function json(data, status = 200) {
  return new Response(JSON.stringify(data, decimalToNumber), {
    status,
    headers: { 'Content-Type': 'application/json' },
  });
}
...
export async function requireRole(allowedRoles) {
  ...
  if (!profile) return { error: jsonError('Non authentifié.e.', 401) };
